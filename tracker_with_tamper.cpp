...
#define RELAY_PIN        4
#define BUZZER_PIN       5
#define TAMPER_PIN       18    // Reed switch for tamper detection
...
// GPS Quality and Movement Thresholds - ADJUSTED FOR YOUR GPS
#define MIN_MOVEMENT_METERS 1.0        // Only send if moved >1 meter
#define GPS_ACCURACY_THRESHOLD 10.0    // Changed to 10.0 (your HDOP is 1.76, so this should pass)
#define GPS_SATELLITE_MIN 3            // Minimum satellites required (you have 8)
#define COORDINATE_SMOOTHING_SAMPLES 5 // Average multiple readings
...
// GPS Quality and Movement Tracking
...
void checkTamper() {
  // For NC wiring: closed (normal) = LOW, opened (tamper) = HIGH
...
// Simplified location function that bypasses smoothing for immediate access
bool currentLocation(double &lat, double &lng) {
  ...
  if (gps.location.isValid() && 
    ...
    Serial.printf("GPS Quality Check PASSED: Valid=%s, Sats=%d, HDOP=%.2f\n", 
  ...
  Serial.printf("GPS Quality Check Failed: Valid=%s, Sats=%d, HDOP=%.2f\n", 
                gps.location.isValid() ? "YES" : "NO",
                gps.satellites.value(),
                gps.hdop.value());
...
// Improved location publishing with movement threshold
void publishLocationOnce() {
  ...
  Serial.printf("GPS Quality: Sats=%d, HDOP=%.2f, Age=%lu ms\n", 
                gps.satellites.value(), 
                gps.hdop.value(), 
                gps.location.age());
...
void ensureNetwork() {
  ...
  if (connectWiFi()) {
    ...
    if (READ_SIM_ON_WIFI || hasSimOverride()) {
      readSimInfo();  // Fixed: was readSimIdentity()
    }
    return;
  }
...
void setup() {
  ...
  pinMode(TAMPER_PIN, INPUT_PULLUP);  // Tamper detection pin
  setImmobilized(false);
  digitalWrite(BUZZER_PIN, LOW);
  ...
  Serial.println(F("=== GPS QUALITY SETTINGS ==="));
  Serial.printf("Min satellites: %d\n", GPS_SATELLITE_MIN);
  Serial.printf("Max HDOP: %.1f\n", GPS_ACCURACY_THRESHOLD);
  Serial.printf("Min movement: %.1f meters\n", MIN_MOVEMENT_METERS);
  Serial.printf("Coordinate smoothing: %d samples\n", COORDINATE_SMOOTHING_SAMPLES);
  Serial.println(F("============================="));
}
...
void loop() {
  ...
  if (millis() - lastGPSStatusTime >= 10000) { // Every 10 seconds
    ...
    if (gps.location.isValid()) {
      ...
      // Force immediate location publish for testing
      Serial.println("Forcing location publish for testing...");
      publishLocationOnce();
    } else {
  ...
  // Heartbeat and location publishing (reduced frequency)
  if (nowMs - lastHeartbeatMs >= HEARTBEAT_INTERVAL_MS) {
    lastHeartbeatMs = nowMs;
    publishHeartbeat();
    
    // Only publish location if we have good GPS and have moved
    if (gps.location.isValid() && 
        gps.satellites.value() >= GPS_SATELLITE_MIN &&
        gps.hdop.value() <= GPS_ACCURACY_THRESHOLD) {
      Serial.println("GPS ready - publishing location...");
      publishLocationOnce();
    } else {
  ...
  // Continuous tracking (only if enabled and good GPS)
  ...
  // Only update geofence if we have good coordinates
  if (gps.location.isUpdated() && 
    ...
    // Log GPS info for debugging
    Serial.printf("GPS Updated: lat=%.6f, lng=%.6f, sats=%u, hdop=%.2f, age=%lu ms\n",
                  lat, lng, gps.satellites.value(), gps.hdop.value(), gps.location.age());
